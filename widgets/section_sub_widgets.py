...
class BaseStaffListWidget(QWidget):
    def __init__(self, parent_widget: TabViewWidget, data: AppData, staff: Staff, tab_name: str, comm_system: BaseCommSystem, card_scanner_index: int, staff_data_index: int):
        ...
        name_label = QLabel(f"{self.staff.name.sur} {self.staff.name.first}, {self.staff.name.middle}")
...
class AttendanceTeacherWidget(BaseAttendanceWidget):
    def __init__(self, data: AttendanceEntry):
        ...
        layout_1_2_2.addWidget(LabeledField("Hr", QLabel(("0" if data.time.hour < 10 else "") + str(data.time.hour))))
...
class StaffListPrefectWidget(BaseStaffListWidget):
    def __init__(self, parent_widget: TabViewWidget, data: AppData, prefect: Prefect, tab_name: str, comm_device: BaseCommSystem, card_scanner_index: int, staff_data_index: int):
        super().__init__(parent_widget, data, prefect, tab_name, comm_device, card_scanner_index, staff_data_index)
        self.container.setProperty("class", "StaffListPrefectWidget")
        
        self.sub_info_layout.addWidget(LabeledField("Post", QLabel(self.staff.post_name)), alignment=Qt.AlignmentFlag.AlignCenter)
        self.sub_info_layout.addWidget(LabeledField("Class", QLabel(self.staff.cls.name)), alignment=Qt.AlignmentFlag.AlignRight)

class StaffListTeacherWidget(BaseStaffListWidget):
    def __init__(self, parent_widget: TabViewWidget, data: AppData, teacher: Teacher, tab_name: str, comm_device: BaseCommSystem, card_scanner_index: int, staff_data_index: int):
        super().__init__(parent_widget, data, teacher, tab_name, comm_device, card_scanner_index, staff_data_index)
        self.container.setProperty("class", "StaffListTeacherWidget")
        
        self.sub_info_layout.addWidget(LabeledField("Dept", QLabel(self.staff.department.name), QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Minimum), alignment=Qt.AlignmentFlag.AlignRight)
